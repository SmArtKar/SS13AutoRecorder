...
namespace SS13AutoRecorder
{
	public partial class TrayMenu : Form
	{
		...
		private void ProcessRecorder(object sender, EventArgs e)
		{
			...
			// TODO: Stop recording after StopDelay dcs if seeker is lost
			...
			OutputState state = OBSHandler.RecordState.Value;
			bool isRecording = 
			...
			if ((isRecording || state == OutputState.OBS_WEBSOCKET_OUTPUT_PAUSED) && status.gamestate == Gamestate.Roundend)
			{
				// TODO: Log roundend
				StopRecording(lastServer?.Name);
				if (state == OutputState.OBS_WEBSOCKET_OUTPUT_STARTING || state == OutputState.OBS_WEBSOCKET_OUTPUT_STARTED || state == OutputState.OBS_WEBSOCKET_OUTPUT_RESUMED)
			...
			if (previousRoundID == -1)
			{
                ...
                if (state != OutputState.OBS_WEBSOCKET_OUTPUT_STARTING && state != OutputState.OBS_WEBSOCKET_OUTPUT_STARTED)
			...
			if (isRecording || state == OutputState.OBS_WEBSOCKET_OUTPUT_PAUSED)
			{
